...
class PowerScalar(TensorOp):
    ...
    def compute(self, a: NDArray) -> NDArray:
        ### BEGIN YOUR SOLUTION
        return array_api.power(a, self.scalar).astype(a.dtype)
        ### END YOUR SOLUTION
...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        for idx in range(len(x)):
            if x[idx] != y[idx]:
                cor.append(idx) 
        return reshape(summation(out_grad, tuple(cor)), lhs.shape)
        ### END YOUR SOLUTION
...
class Summation(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        if self.axes:
            ...
            cor = [1] * len(cor)
        return broadcast_to(reshape(out_grad, cor), lhs.shape)
        ### END YOUR SOLUTION
...
class Negate(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        return -out_grad * array_api.ones(node.inputs[0].shape)
        ### END YOUR SOLUTION
...
class ReLU(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        return out_grad * array_api.where(node.cached_data > 0, 1, 0)
        ### END YOUR SOLUTION
