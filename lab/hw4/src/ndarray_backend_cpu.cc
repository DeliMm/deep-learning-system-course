...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
   ...
   */
  /// BEGIN SOLUTION
  size_t ndim = shape.size(), cnt = 0, size = 1;
  std::vector<int32_t> compact_idxs(shape.size(), 0);
  for (auto v: shape) size *= v;
  while (cnt < size) {
    size_t idx = offset;
    for (int v = 0; v < ndim; v ++) {
      idx += compact_idxs[v] * strides[v];
    }
    out->ptr[cnt] = a.ptr[idx];
    cnt += 1;
    size_t c = 1;
    for (int v = ndim - 1; v >= 0; v --){
      size_t x = compact_idxs[v] + c;
      compact_idxs[v] = x % shape[v];
      c = x / shape[v];
    }
  }
  out -> size = size;
...
void EwiseGe(const AlignedArray& a, const AlignedArray& b, AlignedArray* out) {
  /**
   * Set entries in out to be 1 if the corresponding entries in a is greater than or equal to b, and 0
   * otherwise.
   */
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = a.ptr[i] >= b.ptr[i] ? 1 : 0;
...
void EwiseLog(const AlignedArray& a, AlignedArray* out) {
  /**
   * Set entries in out to be the log of the corresponding entries in a.
   */
  for (size_t i = 0; i < a.size; i++) {
...
void EwiseExp(const AlignedArray& a, AlignedArray* out) {
  /**
   * Set entries in out to be the exp of the corresponding entries in a.
   */
  for (size_t i = 0; i < a.size; i++) {
...
void EwiseTanh(const AlignedArray& a, AlignedArray* out) {
  /**
   * Set entries in out to be the tanh of the corresponding entries in a.
   */
  for (size_t i = 0; i < a.size; i++) {
