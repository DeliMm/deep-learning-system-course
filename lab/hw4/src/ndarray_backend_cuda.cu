...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size){
    size_t idx_a = offset, cnt = gid;
    for (int i = shape.size - 1; i >= 0; i--){
      int t = cnt % shape.data[i];
      idx_a += t * strides.data[i];
      cnt /= shape.data[i];
    }
    out[gid] = a[idx_a];
  }
...
__global__ void ScalarSetitemKernel(scalar_t val, size_t size, scalar_t *out, CudaVec shape, 
                              CudaVec strides, size_t offset){
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size){
    size_t idx_a = offset, cnt = gid;
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray* out, std::vector<int32_t> shape,
  ...
  ScalarSetitemKernel<<<dim.grid, dim.block>>>(val, size, out->ptr, VecToCuda(shape),
                                             VecToCuda(strides), offset);
  /// END SOLUTION
}
...
void ScalarPower(const CudaArray& a, scalar_t val, CudaArray* out) {
  /**
   * Raise a CUDA array to a scalar value.
   */
  CudaDims dim = CudaOneDim(out->size);
...
void EwiseEq(const CudaArray& a, const CudaArray& b, CudaArray* out) {
  /**
   * Check equality of two CUDA arrays
   */
  CudaDims dim = CudaOneDim(out->size);
...
void ScalarEq(const CudaArray& a, scalar_t val, CudaArray* out) {
  /**
   * Check equality of a CUDA array and a scalar value
   */
  CudaDims dim = CudaOneDim(out->size);
...
void EwiseGe(const CudaArray& a, const CudaArray& b, CudaArray* out) {
  /**
   * Check greater than or equal to of two CUDA arrays
   */
  CudaDims dim = CudaOneDim(out->size);
...
void ScalarGe(const CudaArray& a, scalar_t val, CudaArray* out) {
  /**
   * Check greater than or equal to of a CUDA array and a scalar value
   */
  CudaDims dim = CudaOneDim(out->size);
...
void EwiseExp(const CudaArray& a, CudaArray* out) {
  /**
   * Take the exponential of a CUDA array
   */
  CudaDims dim = CudaOneDim(out->size);
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N,
  ...
  size_t x = bx * blockDim.x + tx, y = by * blockDim.y + ty;
...
__global__ void ReduceMaxKernel(const scalar_t *a, scalar_t *out, size_t size, size_t reduce_size){
  ...
  if (gid < size ){
    ...
    for (size_t i = 1; i < reduce_size; i++){
      max_val = fmax(max_val, a[gid * reduce_size + i]);
    }
    out[gid] = max_val;
  }
}
