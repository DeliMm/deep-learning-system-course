...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        for i in range(len(new_shape)):
            if new_shape[i] == -1:
                new_shape[i] = self.size // -prod(new_shape)
    ...
    def permute(self, new_axes):
        ...
        for idx, i in enumerate(new_axes):
            new_shape[idx] = self.shape[i]
            new_strides[idx] = self.strides[i]
    ...
    def broadcast_to(self, new_shape):
        ...
        if (len(new_shape) != len(shape)):
            ...
            strides = [0] * (len(new_shape) - len(strides)) + strides
        for i in range(len(shape)):
            assert new_shape[i] == shape[i] or shape[i] == 1
    ...
    def __getitem__(self, idxs):
        ...
        for idx in idxs:
            st, en, step = idx.start, idx.stop, idx.step
            new_shape.append(1 + (en - st - 1) // step)
            new_strides.append(self.strides[len(new_shape) - 1] * step)
            new_offset += st * self.strides[len(new_shape) - 1]
        return NDArray.make(tuple(new_shape), tuple(new_strides), device=self.device, handle=self._handle, offset=self._offset + new_offset)
        ### END YOUR SOLUTION
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        shape, strides, offset = list(self.shape), list(self.strides), 0
        shape += [1]
        ...
        for t in axes:
            strides[t] = -strides[t]
            offset += (self.shape[t] - 1) * shape[t + 1]
   
        out = NDArray.make(self.shape, strides=tuple(strides), handle=self._handle, device=self.device, offset=offset)
        return out.compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for i in range(len(axes)):
            idx[i] = slice(axes[i][0], shape[i] + axes[i][0])
            shape[i] += axes[i][0] + axes[i][1]
       
        out = NDArray.make(tuple(shape), device=self.device)
        out.fill(0)
        out[tuple(idx)] = self
        return out 
        ### END YOUR SOLUTION
