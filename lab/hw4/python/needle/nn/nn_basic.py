...
class Linear(Module):
    def __init__(
        ...
        if bias:
            self.bias = Parameter(init.kaiming_uniform(fan_in=out_features, fan_out=1, dtype=dtype, device=device).reshape((1, out_features)), requires_grad=True)
        ### END YOUR SOLUTION
...
class SoftmaxLoss(Module):
    def forward(self, logits: Tensor, y: Tensor):
        ...
        rshape = list(logits.shape)
        rshape[-1] = 1
        out = ops.logsumexp(logits, axes=(-1,)).reshape(tuple(rshape)).broadcast_to(logits.shape) - logits
        return (y_one_hot * (out)).sum() / logits.shape[0]
        ### END YOUR SOLUTION
...
class BatchNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            var = ops.summation((x - mean.broadcast_to(x.shape)) ** 2, axes=0) / x.shape[0]
...
class LayerNorm1d(Module):
    def __init__(self, dim, eps=1e-5, device=None, dtype="float32"):
        ...
        self.eps = eps
        ### BEGIN YOUR SOLUTION
        self.weight = Parameter(init.ones(dim, dtype=dtype, device=device, requires_grad=True).reshape((1 , dim)))
        self.bias = Parameter(init.zeros(dim, dtype=dtype, device=device, requires_grad=True).reshape((1, dim)))
        ### END YOUR SOLUTION

    def forward(self, x: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        mean = ops.summation(x, axes=-1).reshape((-1, 1)).broadcast_to(x.shape) / self.dim
        var = ops.summation((x - mean) ** 2, axes=-1).reshape((-1, 1)).broadcast_to(x.shape) / self.dim 
...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        if self.training:
            idx = init.randb((x.shape[-2] * x.shape[-1]), p=1 - self.p, dtype=x.dtype, device=x.device).reshape((x.shape[-2], x.shape[-1]))
            return idx * x / (1 - self.p)
        else:
            return x
        ### END YOUR SOLUTION
